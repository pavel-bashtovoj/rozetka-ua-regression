...
class JasmineAdapter {
    constructor(cid, config, specs, capabilities, reporter) {
        ...
        this.reporter = new reporter_1.default(reporter, {
            cid: this.cid,
    ...
    async init() {
        const self = this;
        this.jrunner = new jasmine_1.default();
        ...
        jasmineEnv.configure({
            ...
            oneFailurePerSpec: Boolean(this.jasmineNodeOpts.stopSpecOnExpectationFailure ||
        ...
        INTERFACES['bdd'].forEach((fnName) => {
            ...
            utils_1.runTestInFiberContext(isTest, isTest ? this.config.beforeTest : beforeHook, hookArgsFn, isTest ? this.config.afterTest : afterHook, hookArgsFn, fnName, this.cid);
        });
    ...
    async run() {
        ...
        await utils_1.executeHooksWithArgs(this.config.after, [result, this.capabilities, this.specs]);
        return result;
    }
    ...
    wrapHook(hookName) {
        return (done) => utils_1.executeHooksWithArgs(this.config[hookName], this.prepareMessage(hookName)).then(() => done(), (e) => {
